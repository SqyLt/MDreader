  ...
  if (window.pywebview) {
    window.pywebview.api.open_file().then(result => {
      if (result.success) {
        ...
        titleInput.value = result.filename.replace(/\.(md|markdown|txt)$/i, "");
        resetHistory();
        render();
        setStatus(t("openedFile", { name: result.filename }));
...
document.querySelector("#saveAsDoc").addEventListener("click", saveAsMarkdown);
...
document.querySelector("#exportPdf").addEventListener("click", () => {
  window.print();
  setStatus(t("exportedFile", { name: normalizeFileName(titleInput.value || currentFileName, "pdf") }));
});
...
editor.addEventListener("keydown", (event) => {
  ...
  if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "s") {
    event.preventDefault();
    if (event.shiftKey) {
      document.querySelector("#saveAsDoc").click();
    } else {
      document.querySelector("#saveDoc").click();
    }
  }
});
...
function saveMarkdown() {
  if (window.pywebview && currentFilePath) {
    window.pywebview.api.save_file(editor.value, currentFilePath).then(result => {
      if (result.success) {
        setStatus(t("savedFile", { name: result.filename }));
      } else {
        setStatus('Error: ' + result.error);
      }
    });
...
function saveAsMarkdown() {
  const name = normalizeFileName(titleInput.value || currentFileName, "md");
  if (window.pywebview) {
    window.pywebview.api.save_as_file(editor.value, name).then(result => {
      if (result.success) {
        currentFileName = result.filename;
        currentFilePath = result.path;
        titleInput.value = result.filename.replace(/\.(md|markdown|txt)$/i, "");
